...
if(onFocus){
	...
	if(keyboard_string != ""){
	    var wText = keyboard_string;
	    var maxHeight = height - (textOffset * 2); // Maximum text height
        var maxLength = width - (textOffset*2);
    
	    draw_set_font(fontType);
	    var lineHeight = string_height("W"); // Height of a single line of text
	    var newText = string_insert(wText, text, cursorPosition); // Potential new text after insertion
		
		
		if string_length(newText) > textMaxCharacters
		{
			keyboard_string = "";
			exit;
		}
	    ...
	    // Calculate the number of lines in the newText
		var maxWidth = width - (inside_margins*2);
		newText = break_text_into_lines_extra(newText, maxWidth);
	    var numLines = string_count("\n", newText) + 2; // +1 because there's always at least one line
	    var totalTextHeight = numLines * lineHeight; // Total height the newText will occupy		
		

	    if totalTextHeight > maxHeight
		{
			var totalTextLength = width_from_last_newline(newText+"W");
	    ...
	    text = newText; // Update the text with the inserted string
	    cursorPosition += string_length(wText); // Update the cursor position
	    keyboard_string = ""; // Clear the keyboard string after processing
	}
}
